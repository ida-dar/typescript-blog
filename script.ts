{
  ...
  const titleClickHandler = function (this: HTMLLinkElement, event: Event): void {
    ...
    const targetArticle = document.querySelector(hrefAttribute) as HTMLLinkElement;
  ...
  const generateTitleLinks = (customSelector: string = ''): void => {
    ...
    const titleList = document.querySelector(selectorTitleList) as HTMLLinkElement;
    titleList.innerHTML = '';
    ...
    const articles: NodeListOf<HTMLElement> = document.querySelectorAll(selectorArticle + customSelector);
    for (let article of articles) {
      ...
      const articleTitle: string | null = article.querySelector(selectorTitle).innerHTML;

      /* create HTML of the link */
      const linkHTML: string | null = '<li><a href="#' + articleID + '"><span>' + articleTitle + '</span></a></li>';
    ...
    const links: NodeListOf<HTMLLinkElement> = document.querySelectorAll('.titles a');
  ...
  const generateTags = (): void => {
    ...
    const articles: NodeListOf<HTMLElement> = document.querySelectorAll(selectorArticle);
    for (let article of articles) {

      /* find div for storing tags  */
      const tagWrapper = article.querySelector(selectorArticleTags) as HTMLDivElement;
  ...
  const generateAuthors = (): void => {
    ...
    for (let article of articles) {
      ...
      const articleAuthor = article.querySelector(selectorArticleAuthor) as HTMLParagraphElement;
